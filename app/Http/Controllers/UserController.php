  ...
  public function index(Request $request)
{
    try {
        ...
        // Appliquer les restrictions en fonction du profil
        if ($user->profil->name === 'Admin général') {
            // Peut tout voir - pas de restriction supplémentaire
        }
        elseif ($user->profil->name === 'Admin compagnie') {
            // Peut voir uniquement les utilisateurs de sa compagnie
            $query->where('idCompagnie', $user->idCompagnie);
        }
        elseif ($user->profil->name === 'Chef de gare') {
            // Peut voir uniquement les utilisateurs de sa gare
            $query->where('idGarre', $user->idGarre);
        }
        // Ajoutez d'autres conditions pour d'autres profils si nécessaire
        ...
        // Récupérer les données pour les filtres avec restrictions
        if ($user->profil->name === 'Admin général') {
            $profils = Profils::all();
            $garres = Garres::all();
        ...
        elseif ($user->profil->name === 'Chef de gare') {
            ...
            // Pour les autres profils, restrictions par défaut
            $profils = Profils::whereNotIn('name', ['Admin général', 'Admin compagnie', 'Chef de gare'])->get();
            $garres = Garres::where('id', $user->idGarre)->get();
            $compagnies = Compagnies::where('id', $user->idCompagnie)->get();
        }
        ...
        // Vous pouvez aussi rediriger avec un message d'erreur
        return back()->with('error', 'Erreur lors de la récupération des utilisateurs.');
    }
}
